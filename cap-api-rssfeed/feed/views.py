            ...
            if categories != [""]:
                for category in categories:
                    try:
                        ...
                        return Response(
                            {"errors": "Invalid category {0}".format(
                                category)}, 400
                        )
        try:
            feed_obj = RSSFeed.objects.get(id=feed_id)
...
class get_articles(APIView):
    ...
    def get(self, request):
        ...
        if categories is not None:
            ...
            for category in categories:
                try:
                    rss_feeds = RSSFeed.objects.filter(
        ...
        for feed in rss_feeds:
            ...
            for article in data["entries"][:count]:
                ...
                article_data["image"] = article.get("image", None)
                article_data["feed_name"] = feed.name
                article_data["categories"] = categories
                articles.append(article_data)
        sorted_articles = list(
            reversed(sorted(articles, key=lambda i: i["date"])))
